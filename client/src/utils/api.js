...
export const fetchPlaylists = async () => {
   ...
   const limit = 50 // Max limit per Spotify API request
   let offset = 0
   let allPlaylists = []
   let hasMore = true
   ...
   while (hasMore) {
      ...
      allPlaylists = allPlaylists.concat(data.items) // Append new playlists

      offset += limit // Move to the next page
      hasMore = data.items.length === limit // Continue if there are more playlists
   }
...
export const addSongToPlaylistAtPosition = async (
   playlistId,
   songUri,
   position
) => {
