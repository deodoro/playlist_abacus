...
import { getUserProfile, getAvailableDevices, setActiveDevice } from "../utils/api";
import { useDevice } from "../context/DeviceContext"; // Import useDevice hook

const UserDeviceSelector = () => {
  const [user, setUser] = useState(null);
  const [devices, setDevices] = useState([]);
  const { activeDeviceId, setActiveDeviceId } = useDevice(); // Use context for activeDeviceId
  ...
  return (
    <div className="p-4">
      ...
      {/* Device Selector */}
      <label htmlFor="deviceSelect" className="block text-sm font-medium text-gray-700">
        Active Device
      </label>
      <select
        id="deviceSelect"
